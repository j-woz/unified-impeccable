...
** Needs `module frameworks/2024` plus `rdkit`
...
** `conda install tensorflow keras mpi4py`
...
`$P/step123`::
`frameworks` plus `pydantic-settings openeye-toolkits`

`$P/plain`::
An initial plain Miniconda 3.12 for cloning

`$P/step4a`::
Python 3.13 plus `mdanalysis`

`$P/step4b`::
Python 3.13 `ambertools`

==== Build the TF tarball

===== For Step 2:

. module load frameworks/2024.?? <- works
..  module load frameworks/2025.0.0
.. . module frameworks/2025.*.0 has a broken mpi4py
. conda clone
. conda install horovod keras tensorflow ipython
. pip install SmilesPE
